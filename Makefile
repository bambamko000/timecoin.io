...
pkgdatadir = $(datadir)/timecore
pkgincludedir = $(includedir)/timecore
pkglibdir = $(libdir)/timecore
pkglibexecdir = $(libexecdir)/timecore
...
CONFIG_HEADER = $(top_builddir)/src/config/time-config.h
CONFIG_CLEAN_FILES = libtimeconsensus.pc share/setup.nsi \
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	...
	$(top_srcdir)/src/config/time-config.h.in \
...
ACLOCAL = ${SHELL} /home/michi/timecoin.io/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
AUTOCONF = ${SHELL} /home/michi/timecoin.io/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /home/michi/timecoin.io/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /home/michi/timecoin.io/build-aux/missing automake-1.15
AWK = gawk
BDB_CPPFLAGS = 
...
MAINT = 
MAKEINFO = ${SHELL} /home/michi/timecoin.io/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
OTOOL64 = 
PACKAGE = timecore
PACKAGE_BUGREPORT = https://github.com/timepay/time/issues
PACKAGE_NAME = TIMECoin Core
PACKAGE_STRING = TIMECoin Core 1.0.0
PACKAGE_TARNAME = timecore
PACKAGE_URL = 
...
ZMQ_CFLAGS = 
ZMQ_LIBS = -lzmq
abs_builddir = /home/michi/timecoin.io
abs_srcdir = /home/michi/timecoin.io
abs_top_builddir = /home/michi/timecoin.io
abs_top_srcdir = /home/michi/timecoin.io
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /home/michi/timecoin.io/build-aux/install-sh
...
pkgconfig_DATA = libtimeconsensus.pc
BITCOIND_BIN = $(top_builddir)/src/timed$(EXEEXT)
BITCOIN_QT_BIN = $(top_builddir)/src/qt/time-qt$(EXEEXT)
BITCOIN_CLI_BIN = $(top_builddir)/src/time-cli$(EXEEXT)
BITCOIN_WIN_INSTALLER = $(PACKAGE)-$(PACKAGE_VERSION)-win$(WINDOWS_BITS)-setup$(EXEEXT)
OSX_APP = TIMECoin-Qt.app
OSX_DMG = TIMECoin-Core.dmg
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info block_test.info \
  leveldb_baseline.info test_time_filtered.info total_coverage.info \
  baseline_filtered.info block_test_filtered.info rpc_test.info rpc_test_filtered.info \
  leveldb_baseline_filtered.info test_time_coverage.info test_time.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/TIMECoin-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/time-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/time-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/time-config.h
$(top_srcdir)/src/config/time-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/time-config.h src/config/stamp-h1
libtimeconsensus.pc: $(top_builddir)/config.status $(srcdir)/libtimeconsensus.pc.in
...
$(OSX_APP)/Contents/MacOS/TIMECoin-Qt: $(BITCOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/TIMECoin-Qt

$(OSX_DMG): $(APP_DIST_EXTRAS)
	$(GENISOIMAGE) -no-cache-inodes -D -l -probe -V "TIMECoin-Core" -no-pad -r -dir-mode 0755 -apple -o $@ dist
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/TIMECoin-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_time.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_time -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_time_filtered.info: test_time.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#block_test.info: test_time_filtered.info
#	$(MKDIR_P) qa/tmp
#	-@TIMECOUT=15 qa/pull-tester/run-bitcoind-for-test.sh $(JAVA) -jar $(JAVA_COMPARISON_TOOL) qa/tmp/compTool $(COMPARISON_TOOL_REORG_TESTS)
...
#rpc_test.info: test_time_filtered.info
#	-@TIMECOUT=15 python qa/pull-tester/rpc-tests.py $(EXTENDED_RPC_TESTS)
...
#test_time_coverage.info: baseline_filtered_combined.info test_time_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_time_filtered.info -o $@

#total_coverage.info: baseline_filtered_combined.info test_time_filtered.info block_test_filtered.info rpc_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_time_filtered.info -a block_test_filtered.info -a rpc_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_time.coverage/.dirstamp:  test_time_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_time.coverage/.dirstamp total.coverage/.dirstamp

# TIMECoin: disable the test until upstream switches from convoluted comparison tool
...
clean-local:
	rm -rf coverage_percent.txt test_time.coverage/ total.coverage/ qa/tmp/ cache/ $(OSX_APP)
