...
*Setup instructions for a Gitian build of TIMECoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the TIMECoin
...
- [Building TIMECoin Core](#building-time-core)
...
Clone the git repositories for TIMECoin Core and Gitian.
...
in the TIMECoin Core repository under 'Fetch and build inputs' to install sources which require
...
To build TIMECoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the TIMECoin Core repository.
