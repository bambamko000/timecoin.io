...
SEQUENCE_LOCKTIMEC_DISABLE_FLAG = (1<<31)
SEQUENCE_LOCKTIMEC_TYPE_FLAG = (1<<22) # this means use time (0 means height)
SEQUENCE_LOCKTIMEC_GRANULARITY = 9 # this is a bit-shift
SEQUENCE_LOCKTIMEC_MASK = 0x0000ffff
...
class BIP68Test(BitcoinTestFramework):
    ...
    def test_disable_flag(self):
        ...
        # input to mature.
        sequence_value = SEQUENCE_LOCKTIMEC_DISABLE_FLAG | 1
        ...
        for i in xrange(400):
            ...
            for j in xrange(num_inputs):
                ...
                if random.randint(0,1):
                    ...
                    can_time_lock = True
                    if ((cur_time - orig_time) >> SEQUENCE_LOCKTIMEC_GRANULARITY) >= SEQUENCE_LOCKTIMEC_MASK:
                        can_time_lock = False

                    # if time-lockable, then 50% chance we make this a time lock
                    if random.randint(0,1) and can_time_lock:
                        # Find first time-lock value that fails, or latest one that succeeds
                        time_delta = sequence_value << SEQUENCE_LOCKTIMEC_GRANULARITY
                        if input_will_pass and time_delta > cur_time - orig_time:
                            sequence_value = ((cur_time - orig_time) >> SEQUENCE_LOCKTIMEC_GRANULARITY)
                        elif (not input_will_pass and time_delta <= cur_time - orig_time):
                            sequence_value = ((cur_time - orig_time) >> SEQUENCE_LOCKTIMEC_GRANULARITY)+1
                        sequence_value |= SEQUENCE_LOCKTIMEC_TYPE_FLAG
