...
static void RelayAddress(const CAddress& addr, bool fReachable, CConnman& connman)
{
    ...
    auto sortfunc = [&mapMix, &hashRand](CNode* pnode) {
        if (pnode->nVersion >= CADDR_TIMEC_VERSION) {
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTIMECoinReceived, CConnman& connman, std::atomic<bool>& interruptMsgProc)
{
    ...
    if (strCommand == NetMsgType::VERSION)
    {
        ...
        if (!pfrom->fInbound)
        {
            ...
            if (pfrom->fOneShot || pfrom->nVersion >= CADDR_TIMEC_VERSION || connman.GetAddressCount() < 1000)
            {
    ...
    else if (strCommand == NetMsgType::ADDR)
    {
        ...
        if (pfrom->nVersion < CADDR_TIMEC_VERSION && connman.GetAddressCount() > 1000)
            return true;
        if (vAddr.size() > 1000)
        {
            LOCK(cs_main);
