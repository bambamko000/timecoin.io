...
static const int SOCKS5_RECV_TIMECOUT = 20 * 1000;
...
static bool Socks5(const std::string& strDest, int port, const ProxyCredentials *auth, SOCKET& hSocket)
{
    ...
    char pchRet1[2];
    if (!InterruptibleRecv(pchRet1, 2, SOCKS5_RECV_TIMECOUT, hSocket)) {
        CloseSocket(hSocket);
    ...
    if (pchRet1[1] == 0x02 && auth) {
        ...
        char pchRetA[2];
        if (!InterruptibleRecv(pchRetA, 2, SOCKS5_RECV_TIMECOUT, hSocket)) {
            CloseSocket(hSocket);
    ...
    char pchRet2[4];
    if (!InterruptibleRecv(pchRet2, 4, SOCKS5_RECV_TIMECOUT, hSocket)) {
        CloseSocket(hSocket);
        return error("Error reading proxy response");
    }
    if (pchRet2[0] != 0x05) {
    ...
    switch (pchRet2[3])
    {
        case 0x01: ret = InterruptibleRecv(pchRet3, 4, SOCKS5_RECV_TIMECOUT, hSocket); break;
        case 0x04: ret = InterruptibleRecv(pchRet3, 16, SOCKS5_RECV_TIMECOUT, hSocket); break;
        case 0x03:
        {
            ret = InterruptibleRecv(pchRet3, 1, SOCKS5_RECV_TIMECOUT, hSocket);
            if (!ret) {
                CloseSocket(hSocket);
                return error("Error reading from proxy");
            }
            int nRecv = pchRet3[0];
            ret = InterruptibleRecv(pchRet3, nRecv, SOCKS5_RECV_TIMECOUT, hSocket);
            break;
        }
    ...
    if (!InterruptibleRecv(pchRet3, 2, SOCKS5_RECV_TIMECOUT, hSocket)) {
        CloseSocket(hSocket);
        return error("Error reading from proxy");
    }
