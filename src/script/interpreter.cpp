    ...
    try
    {
        while (pc < pend)
        {
            ...
            switch (opcode)
            {
                ...
                case OP_CHECKLOCKTIMECVERIFY:
                {
                    if (!(flags & SCRIPT_VERIFY_CHECKLOCKTIMECVERIFY)) {
                ...
                case OP_CHECKSEQUENCEVERIFY:
                {
                    ...
                    if ((nSequence & CTxIn::SEQUENCE_LOCKTIMEC_DISABLE_FLAG) != 0)
                        break;
...
bool TransactionSignatureChecker::CheckLockTIMECCoin(const CScriptNum& nLockTIMECCoin) const
{
    ...
    // required to prove correct CHECKLOCKTIMECVERIFY execution.
...
bool TransactionSignatureChecker::CheckSequence(const CScriptNum& nSequence) const
{
    ...
    if (txToSequence & CTxIn::SEQUENCE_LOCKTIMEC_DISABLE_FLAG)
        return false;
    ...
    // and lock-by-blocktime, distinguished by whether
    // nSequenceMasked < CTxIn::SEQUENCE_LOCKTIMEC_TYPE_FLAG.
    //
    // We want to compare apples to apples, so fail the script
    // unless the type of nSequenceMasked being tested is the same as
    // the nSequenceMasked in the transaction.
    if (!(
        (txToSequenceMasked <  CTxIn::SEQUENCE_LOCKTIMEC_TYPE_FLAG && nSequenceMasked <  CTxIn::SEQUENCE_LOCKTIMEC_TYPE_FLAG) ||
        (txToSequenceMasked >= CTxIn::SEQUENCE_LOCKTIMEC_TYPE_FLAG && nSequenceMasked >= CTxIn::SEQUENCE_LOCKTIMEC_TYPE_FLAG)
    )) {
        return false;
    }
