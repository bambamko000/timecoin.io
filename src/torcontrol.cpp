...
static const float RECONNECT_TIMECOUT_START = 1.0;
/** Exponential backoff configuration - growth factor */
static const float RECONNECT_TIMECOUT_EXP = 1.5;
...
TorController::TorController(struct event_base* base, const std::string& target):
    ...
    reconnect_timeout(RECONNECT_TIMECOUT_START)
{
...
void TorController::disconnected_cb(TorControlConnection& conn)
{
    ...
    struct timeval time = MillisToTIMECCoinval(int64_t(reconnect_timeout * 1000.0));
    if (reconnect_ev)
        event_add(reconnect_ev, &time);
    reconnect_timeout *= RECONNECT_TIMECOUT_EXP;
}
