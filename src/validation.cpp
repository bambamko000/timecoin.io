...
static std::pair<int, int64_t> CalculateSequenceLocks(const CTransaction &tx, int flags, std::vector<int>* prevHeights, const CBlockIndex& block)
{
    ...
    bool fEnforceBIP68 = static_cast<uint32_t>(tx.nVersion) >= 2
                      && flags & LOCKTIMEC_VERIFY_SEQUENCE;
    ...
    for (size_t txinIndex = 0; txinIndex < tx.vin.size(); txinIndex++) {
        ...
        if (txin.nSequence & CTxIn::SEQUENCE_LOCKTIMEC_DISABLE_FLAG) {
                              ...
                              std::vector<COutPoint>& coins_to_uncache, bool fDryRun)
{
...

    {
        ...
        if (!CheckSequenceLocks(tx, STANDARD_LOCKTIMEC_VERIFY_FLAGS, &lp))
...
{
  CAmount ret = blockValue*0.5;    // 50% MN payout
...
static bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck = false)
{
    ...
    // Start enforcing CHECKLOCKTIMECVERIFY, (BIP65) for block.nVersion=4
    // blocks, when 75% of the network has upgraded:
    if (block.nVersion >= 4 && IsSuperMajority(4, pindex->pprev, chainparams.GetConsensus().nMajorityEnforceBlockUpgrade, chainparams.GetConsensus())) {
        flags |= SCRIPT_VERIFY_CHECKLOCKTIMECVERIFY;
    }
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(TIMEC): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }
...
static bool ActivateBestChainStep(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexMostWork, const CBlock* pblock, bool& fInvalidFound)
{
    ...
    if (fBlocksDisconnected) {
        mempool.removeForReorg(pcoinsTip, chainActive.Tip()->nHeight + 1, STANDARD_LOCKTIMEC_VERIFY_FLAGS);
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // TIMEC : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        LogPrintf("CheckBlock(TIMEC): spork is off, skipping transaction locking checks\n");
    }
